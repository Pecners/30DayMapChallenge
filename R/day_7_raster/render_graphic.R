...
d_ <- raster("data/michigan_lld/michigan_lld.tif")

mat <- raster_to_matrix(d_)
mat[mat > 0 ] <- NA
mat <- abs(mat)
...
# GitHub difficult. 
outfile <- str_to_lower(glue("plots/day_7/michigan_{pal}.png"))
...
saveRDS(list(
  ...
  outfile = outfile,
  coords = coords
), glue("R/day_7_raster/header.rds"))
