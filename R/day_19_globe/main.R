...
ggsave(loc_plot, filename = "plots/day_19/congaree_inset.png", w = 4*1.5, h = 3*1.5)

img <- image_read("plots/day_19/congaree_inset.png")
info <- image_info(img)
...
sc |> 
  ...
  geom_sf_text(aes(label = str_to_upper(NAME)), color = "grey50", family = "Skia",
               nudge_y = 110000, nudge_x = -100000, size = 10) +
  ...
  geom_sf_text(data = columbia, aes(label = NAME), color = "grey50", size = 10,
               family = "Skia", nudge_x = 45000, nudge_y = 15000) +
  coord_sf(crs = prj, clip = "off") +
  theme_void()

ggsave(glue("plots/day_19/sc_inset.png"))

sc_img <- image_read(glue("plots/day_19/sc_inset.png"))


both <- image_blank(width = image_info(sc_img)$width, height = image_info(sc_img)$height) |> 
  image_composite(image_scale(sc_img, glue("{.5*image_info(sc_img)$width}")), operator = "plus",
                  gravity = "northeast") |> 
  image_composite(image_scale(out, "75%"), operator = "plus", gravity = "center", 
                  offset = "-1000+0") |> 
  image_background(color = "white") |> 
  image_crop(geometry = "1750x2000+0-500", gravity = "east")

image_write(both, glue("plots/day_19/final_inset.png"))
