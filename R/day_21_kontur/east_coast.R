...
ec_r <- st_intersection(data, east_coast)
ec_r <- st_transform(ec_r, crs = 5070)
...
outfile <- "plots/day_21/east_coast_pop.png"
...
annot <- glue("This map shows population density within ten miles ",
              "of the eastern coastline. Population estimates are bucketed ",
              "into 400 meter hexagons.") |> 
  str_wrap(40)

img |> 
  image_crop(geometry = "4500x4500+0+200", gravity = "center") |> 
  ...
  image_annotate(text = "Population Density", gravity = "northwest",
                 location = "+400+800", font = "El Messiri",
                 color = c1[1],
                 size = 250) |> 
  ...
  image_composite(image_modulate(f_image, saturation = 75) |> 
                  ...
                  gravity = "south", 
                  offset = "+0+100") |> 
  image_write("plots/day_21/titled_east_coast_pop.png")
