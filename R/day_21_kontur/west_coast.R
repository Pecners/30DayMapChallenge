...
coast <- ne_download(type = "coastline", category = "physical", scale = "large")  %>%
  st_as_sf() %>%
  st_transform(., crs = st_crs(data))
...
wc_r <- st_intersection(data, west_coast)
wc_r <- st_transform(wc_r, crs = 32610)
...
outfile <- "plots/day_21/west_coast_pop.png"
...
read_lines("R/day_21_kontur/CompassRose.svg") |> 
  gsub(x = _, pattern = "#326812", c1[2]) |> 
  gsub(x = _, pattern = "#99ce64", c1[4]) |> 
...
annot <- glue("This map shows population density within ten miles ",
              "of the western coastline. Population estimates are bucketed ",
              "into 400 meter hexagons.") |> 
  str_wrap(40)

img |> 
  image_crop(geometry = "5500x5250+1000+200", gravity = "center") |> 
  image_annotate(text = "US Western Coastline", gravity = "northeast",
                 location = "+500+700", font = "El Messiri",
                 color = c1[1],
                 size = 250, weight = 700) |> 
  ...
  image_annotate(text = annot, gravity = "northeast",
                 location = "+500+1800", font = "El Messiri",
                 color = alpha(c1[1], .75),
                 size = 125) |> 
  ...
  image_composite(image_modulate(f_image, saturation = 75) |> 
                  ...
                  gravity = "south", 
                  offset = "+1000+1000") |> 
  image_write("plots/day_21/titled_west_coast_pop.png")
