...
wi_d <- st_intersection(data, wi)


size <- 2500
rast <- st_rasterize(wi_d |> 
                       select(population, geom),
                     nx = size, ny = size)


mat <- matrix(rast$population, nrow = size, ncol = size)

pal <- "pink_greens"

c1 <- prettycols("PinkGreens")
colors <- c(c1[c(6:9, 1:4)])
swatchplot(colors)
...
render_camera(phi = 60, zoom = .75, theta = -20)

outfile <- "plots/day_21/wi_pop.png"
...
img |> 
  ...
  image_composite(image_modulate(f_image, saturation = 25) |> 
                  ...
                  gravity = "south", 
                  offset = "-1400+0") |> 
  image_write("plots/day_21/titled_wi_pop.png")
