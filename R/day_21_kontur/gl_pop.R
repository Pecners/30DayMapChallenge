...
gl_d <- st_intersection(data, gl_buff)
gl_d <- gl_d |> 
  st_transform(crs = 3174)
...
outfile <- "plots/day_21/gl_pop.png"
...
annot <- glue("This map shows US population density within ten miles ",
              "of the Great Lakes. Population estimates are bucketed ",
              "into 400 meter hexagons.") |> 
  str_wrap(40)

img |> 
  image_crop(geometry = "5000x3750", gravity = "center") |> 
  image_annotate(text = "US Shores of the Great Lakes", gravity = "northwest",
  ...
  image_annotate(text = annot, gravity = "northwest",
                 location = "+2700+1300", font = "El Messiri",
                 color = alpha(c1[1], .75),
                 size = 100) |> 
  ...
  image_composite(image_modulate(f_image, saturation = 75) |> 
                  ...
                  gravity = "south", 
                  offset = "-1700+100") |> 
  image_write("plots/day_21/titled_gl_pop.png")
