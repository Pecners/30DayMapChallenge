...
hex_summed <- read_rds("R/day_6_asia/hex_summed.rda") |> 
  filter(!is.na(total))
wi_skinny <- read_rds("R/day_6_asia/wi_skinny.rda")
wi_skinny <- wi_skinny |> 
  st_cast("MULTILINESTRING") |> 
  st_buffer(1609.34 * .25) |> 
  mutate(total = 1)

st_d <- bind_rows(wi_skinny, hex_summed)
...
rast <- st_rasterize(st_d |> 
                       select(total, geometry),
                     nx = floor(size * x_rat), ny = floor(size * y_rat))


mat <- matrix(rast$total, nrow = floor(size * x_rat), ncol = floor(size * y_rat))
...
texture <- grDevices::colorRampPalette(colors[6])(256)
...
render_camera(phi = 45, zoom = .85, theta = 0)
...
# GitHub difficult. 
outfile <- "temp/day_6_asia/render.png"
