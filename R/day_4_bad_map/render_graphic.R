...
colors <- grDevices::colorRampPalette(colors = c1, bias = 1.5)(256)
...
# setting resolution
res <- mean(round(terra::res(zelev))) / 3
...
render_camera(phi = 45, zoom = 1, theta = 15)
...
if (!dir.exists(glue("plots/day_4_bad_map"))) {
  dir.create(glue("plots/day_4_bad_map"))
}
...
# GitHub difficult. 
outfile <- str_to_lower(glue("plots/day_4_bad_map/{pal}_z{z}.png"))
...
saveRDS(list(
  ...
  outfile = outfile,
  coords = coords
), glue("R/day_4_bad_map/header.rds"))

{
  ...
  render_highquality(
    ...
    environment_light = "temp/env/phalzer_forest_01_4k.hdr",
